      ...
      CallExpression(node) {
        ...
        context.report({
          ...
          fix(fixer) {
            ...
            const queryKeyProperty = queryKey === 'queryKey' ? 'queryKey' : `queryKey: ${queryKey}`;
            ...
            if (secondArgument && secondArgument !== optionsObject) {
              ...
              const queryFnProperty = queryFn === 'queryFn' ? 'queryFn' : `queryFn: ${queryFn}`;
              optionsObjectProperties.push(queryFnProperty);
            }

            // options
            if (optionsObject) {
              const existingObjectProperties = optionsObject.properties.map((objectLiteral) => {
                return sourceCode.getText(objectLiteral);
              });
              optionsObjectProperties.push(...existingObjectProperties);
            }

            const argumentsRange = getRangeOfArguments(node);
            if (argumentsRange) {
